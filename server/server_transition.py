...
URLSRC = "http://tinywebdb.appinventor.space/api"
USER = "share"
SECRET = "everyone"

SDATA_PRE = "gtcs_"

URLCALL = URLSRC + "?user=" + USER + "&secret=" + SECRET

def td_update(tag, value):
    global URLCALL
    ul = urlopen(URLCALL + "&action=update&tag=" + str(tag) + "&value=" + str(value))
    ul.read()
    ul.close()

def td_query(tag):
    global URLCALL
    ul = urlopen(URLCALL + "&action=get&tag=" + str(tag))
    s = ul.read().decode('utf-8')
    ul.close()
    return json.loads(s)

def td_search(tag, no, count):
    global URCALL
    ul = urlopen(URLCALL + "&action=search&no={}&count={}&tag={}".format(no,count,tag))
    s = ul.read().decode('utf-8')
    ul.close()
    try:
        return json.loads(s)
...
# Authorization test:
utest = urlopen(URLCALL + "&action=count")
if utest.read(2).decode('utf-8')[0] != "{":
    utest.close()
    print("Bad user, secret code or database configuration. Server can't start!")
    exit(1)
utest.close()

app = Flask("FountaineMTR-Comp")
...
def report_signal_mod(key):
    global sig_last_mod
    sig_last_mod[key] = time.time()

# Single update process. 1s each time, I assume
def sig_update():
    global SDATA_PRE, signals, originals, sig_last_mod, manuals
    ...
    try:
        ...
        for i in signals:
            ...
            try:
                ...
                # Fatui HAS BEEN DAMAGING DATABASE. Fix any corruption detected.
                if type(fetched[hauptext]) is not list:
                    uploading = True
                    print("[Err 1] Data corruption for",i)
                elif len(fetched[hauptext]) < 4:
                    uploading = True
                    print("[Err 2] Data corruption for",i)
                if not uploading:
                ...
                if uploading:
                    ...
                    originals[i] = signals[i][2]
                    if signals[i][2] not in DANGEROUS:
                        manuals[i] = signals[i][2]
                    report_signal_mod(i)
                    print("Signal downloaded:",i)
            except Exception as e:
                print("Single signal process failed",e,"; now forced to upload",i,"to override")
                td_update(hauptext, urllib.parse.quote(uploadtext,""))
                st = time.time()
                sig_last_mod[i] = st
                td_update(zeitext, str(st))
    except Exception as e:
...
if __name__ == '__main__':
    ...
    print("""
**************************************************
You are running a TinyWebDB-compatible server.
This server DOES NOT support authorization.
Unless due to technical difficulties, DO NOT use
it for productive environment, unless Jean Gunnhildr
asked you to do so.
**************************************************
""")
    t_tst = threading.Thread(target=sigthread)
    t_tst.start()
